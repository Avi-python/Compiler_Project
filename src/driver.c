...
int main(int argc, char **argv) 
{
    ...
    {
        int semantic_errors = semantic_analysis(result.ast_root, result.global_sym_table);
    } 
    else 
    {
    ...
    if (result.error_count + semantic_error_count > 0) 
    {
    ...
    printf("--- Driver: Execution Finished ---\n");
    printf("Number of errors: %d\n", result.error_count + semantic_error_count);

    if (result.ast_root) visualize_ast(result.ast_root, "ast_output.dot");
    else                 printf("AST Root is NULL.\n");
