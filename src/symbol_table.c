...
void show_symbol_table_recursive(sym_t* sym_table, int indent_level, FILE* log_file) 
{
    ...
    {
        fprintf(log_file, "  "); // 2 spaces per indent level
    }

    fprintf(log_file, "--- Scope (Depth: %d, Addr: %p, Parent: %p, Children: %d) ---\n",
           sym_table->depth, (void*)sym_table, (void*)sym_table->parent, sym_table->num_children);
    
    if (sym_table->table) 
    {
        ht_show(sym_table->table, indent_level + 1, log_file); 
    } 
    else 
    {
        for (int i = 0; i < indent_level + 1; ++i) fprintf(log_file, "  ");
        fprintf(log_file, "(Hash table not initialized)\n");
    }

    // Recursively show children
    for (int i = 0; i < sym_table->num_children; ++i) 
    {
        show_symbol_table_recursive(sym_table->children[i], indent_level + 1, log_file);
    }
}
...
void show_entire_symbol_tree(sym_t* root_sym_table, FILE* log_file) 
{
    ...
    show_symbol_table_recursive(root_sym_table, 0, log_file);
    fprintf(log_file, "=======================================\n\n");
}
...
void show_current_and_parent_scopes(sym_t* current_scope, FILE* log_file) 
{
    ...
    while(temp != NULL) 
    {
        ...
        if (temp->table) 
        {
            ht_show(temp->table, indent + 1, log_file); // Pass indent level to ht_show
        } 
        else 
        {
