...
void ht_show(ht_t* table, int indent_level, FILE* log_file) 
{
    if (!table) 
    {
        fprintf(log_file, "%*sHash table is NULL.\n", indent_level * 2, "");
        return;
    }
    fprintf(log_file, "%*sHash Table Contents (Size: %u, Count: %u):\n", indent_level * 2, "", table->size, table->count);
    for (unsigned int i = 0; i < table->size; i++) 
    {
        ht_entry_t *entry = table->entries[i];
        if (entry != NULL) 
        {
            fprintf(log_file, "%*sBucket %u: ", (indent_level + 1) * 2, "", i);
            while (entry != NULL) 
            {
                fprintf(log_file, "[\"%s\" -> Symbol(Tag: %d, Name: %s)] ", entry->key, entry->value->tag, entry->value->name); 
                if (entry->next != NULL) 
                {
                    fprintf(log_file, "-> ");
                }
